...
class HistoryRevisionsPageProcessor(HistoryPageProcessor):
    ...
    # HERE I SHOULD do something with the current self
    #in particular self._text contains the text of the entire revision. print it?
    ...
    def process_title(self, elem):
        super(HistoryRevisionsPageProcessor,self).process_title(elem)
        if not self._skip:
            logging.debug("### elem=" + self._title) #        logging.debug("### elem=" + elem.text.encode('ascii','ignore'))
...
def main():
    ...
    try:
        lang_talk = unicode(translation['Talk'])
    except UnicodeDecodeError:
        lang_talk = smart_str(translation['Talk'])

    assert lang_talk, "Talk namespace not found"

    src.close()
    src = deflate(xml)

    processor = HistoryRevisionsPageProcessor(tag=tag, lang=lang)
    #                                          desired_names=(lang_talk, u"Talk"))
