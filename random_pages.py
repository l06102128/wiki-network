...
class HistoryRevisionsPageProcessor(HistoryPageProcessor):
    ...
    def end(self):
        logging.info('PAGES: %d - SAVED: %d' % (self.counter_pages,
                                                self.counter_saved))
        self.save()
    ...
    def process_title(self, elem):
        ...
        #clean attributes
        self.delattr(("_counter", "_type", "_title", "_editors",
                      "_initial_revision"))
    ...
    def process_text(self, elem):
        if self._skip:
            return
        self._counter_revisions += 1
        if self.min_text and \
            len(elem.text) < self.min_text:
            if self._desired:
                logging.warning("Desired page %s skipped due to " \
                                "its text size: %d" %
    ...
    def process_timestamp(self, elem):
        ...
        if self.start_revision and not self._initial_revision:
            ...
            if revision_time > self.start_revision:
                if self._desired:
                    logging.warning("Desired page %s skipped due to " \
                                    "its initial revision date: %s" %
                                    (self._title,
                                     revision_time.strftime('%Y-%m-%d')))
                self._skip = True
                return
    ...
    def process_page(self, elem):
        ...
        if not self._skip and self.n_users and \
           len(self._editors) < self.n_users:
            if self._desired:
                logging.warning("Desired page %s skipped due to low number "
                                "of editors: %d" %
                                (self._title, len(self._editors)))
            self._skip = True

        if not self._skip and self.min_revisions and \
           self._counter_revisions < self.min_revisions:
            if self._desired:
                logging.warning("Desired page %s skipped due to low number "
                                "of revisions: %d" %
                                (self._title, len(self._editors)))
...
def dumps_checker(args, type_):
    ...
    if args.editors_number or args.initial_revision or args.revisions_number:
        assert re.search('.-(meta-history)', type_), \
...
def create_option_parser():
    ...
    p = argparse.ArgumentParser(description='Extract a random sample of pages '
                                            'title out of a Wikipedia dump')
    ...
    p.add_argument('xml_fn', help="wikipedia dump to be parsed",
                   metavar="DUMP_FILE")
    p.add_argument('desired_pages_fn',
                   help="csv file containing desired pages",
                   metavar="DESIRED_PAGES_FILE")
    return p
